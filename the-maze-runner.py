...
def pause_menu():
    while True:
        ...
        sub = FONT_SMALL.render("Press ESC to resume", True, YELLOW)
...
def main_menu():
    while True:
        ...
        title = FONT_BIG.render("THE MAZE RUNNER", True, GREEN)
        start = FONT_MED.render("START GAME", True, WHITE)

        btn_rect = pygame.Rect(WIDTH//2 - 120, 400, 240, 60)

        pygame.draw.rect(screen, (50,90,150), btn_rect, border_radius=10)

        screen.blit(title, (WIDTH//2 - title.get_width()//2, 200))
        screen.blit(start, (btn_rect.centerx - start.get_width()//2,
                            btn_rect.centery - start.get_height()//2))
